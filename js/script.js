...
function countPromocionesParaSucursal(sucursalId) {
    ...
    promocionesData.forEach((row, index) => {
        // Saltar filas 18 y 19 para todas las sucursales (índices 17 y 18)
        if (index === 17 || index === 18) return;
        
        // Solo contar filas 3-20 (índices 2-19), excluyendo 18 y 19
        if (index >= 2 && index <= 19) {
            const columnIndex = getColumnIndex(sucursal.column);
            if (row[columnIndex] && row[columnIndex].toString().trim().toLowerCase() === 'x') {
                count++;
            }
        }
    });
